...
void ETH_MACClearITPendingBit(uint32_t ETH_MAC_IT)
{
  /* Check the parameters */
  assert_param(IS_ETH_MAC_IT(ETH_MAC_IT));
  
  /* Clear the selected ETHERNET DMA IT */
  ETH->MACSR = (uint32_t) ETH_MAC_IT;
}
...
void ETH_EnablePTPALLTSnapshot (void)
{
  ETH->PTPTSCR |= ETH_PTPTSSR_TSSARFE;  //使能内核所接收的全部帧
}

/**
  * @brief  : PTP over ethernet  (Time stamp snapshot for PTP over ethernet frames enable)
  * @param  None
  * @retval None
  */

void ETH_EnablePTPOverEthernet (void)
{
  ETH->PTPTSCR |= ETH_PTPTSSR_TSSPTPOEFE;  //使能PTP over ethernet
}

/**
  * @brief  : Time stamp snapshot for event message enable  SYNC Delay_Re Pdelay_Req Pdelay_Resp
  * @param  None
  * @retval None
  */

void ETH_EnablePTPEventMsg(void)
{
...
void ETH_EnableIPv4Snapshot(void)
{
...
void ETH_EnableIPv6Snapshot(void)
{
  ETH->PTPTSCR |= ETH_PTPTSSR_TSSIPV6FE;  
}
...
/**
  ...
  * @retval None
  */
void ETH_EnableListenPTPData(void)
{
...
void ETH_SetPTPTargetTime(uint32_t HighValue, uint32_t LowValue)
{
  ...
  ETH->PTPTTLR=ETH_PTPNanoSecond2SubSecond(LowValue);    
}
...
/**
  ...
  * @retval None
  */
void ETH_SetPTPTargetInitTime(uint32_t HighValue, uint32_t LowValue)
{
  /* Set the PTP Target Time High Register */
  ETH->PTPTSHR = HighValue;
  /* Set the PTP Target Time Low Register */
  //ETH->PTPTTLR = LowValue;
  ETH->PTPTSLR=ETH_PTPNanoSecond2SubSecond(LowValue);    
}
