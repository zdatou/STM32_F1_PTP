...
{										    
 	u8 ublox_uart_state;	//使能输出t302串口输出  1：输出ublox 协议  0:不输出ublox协议     信息采用u-center进行配置
	u8 pps_enable_state;//输出可用1pps :1 不可用输出 ：0
	u8 leap_state_state;//0为无闰秒警告  1：为加一秒  2：为减一秒
	u8 tod_state ; //0：是不输出tod  1：是输出TOD TOD消息和ublox 消息不能同时输出
	u8 edge_tod_state;
	u8 lock_state ; // 0:是没锁定  1：是锁定
	u8 set_fre_state ;	   
}msgstate;  
...
{										    
	...
	u8 posmode;         //定位模式指示 A：自主 D:差分 E：估算 M：手动 S：模拟器模式 N：数据无效
	u8 posstate;        //定位有效/无效
